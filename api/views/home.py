...
def user_notification_list(request, format=None):
	...
	if query == 'all':
		result = Notification.objects.filter(user_id=data['user_id']).order_by('-time_date')
	else:
		label = _NOTIFICATION_LABEL_DIC.get(query, None)
		if label is not None:
			result = Notification.objects.filter(user_id=data['user_id']).filter(label=label
				).order_by('-time_date')
...
def user_pinned_product_list(request, format=None):
	...
	result = PinnedProduct.objects.filter(user_id=data['user_id']).order_by('-time_date')
